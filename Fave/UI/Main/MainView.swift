    ...
    var body: some View {
        NavigationView(content: {
            mainViewList
                ...
                .navigationBarTitle(Text("Top Stories"), displayMode:.large)
        })
    }
        ...
        List {
            ...
            Section(header: Text(verbatim: "Categories")) {
                    ForEach(categories, id: \.self) { category in
                        NavigationLink(
                            destination: ArticleFromCategoryView(category: category).navigationBarTitle(Text(category), displayMode: .large)
                               ) {
                                Text(category)
                               }
                           }
